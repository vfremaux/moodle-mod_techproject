...
function toggle(i, n, ajax) {
    if ($('#' + n).css('display') === 'none') {
        ...
        if (ajax) {
            params = 'id=' + moodlevars.cmid + '&entity=' + moodlevars.view + '&userid=' + moodlevars.userid + '&state=0&entryid=' + i;
            var sUrl = M.cfg.wwwroot + '/mod/techproject/ajax/updatecollapse.php?' + params;
        ...
        if (ajax) {
            params = 'id=' + moodlevars.cmid + '&entity=' + moodlevars.view + '&userid=' + moodlevars.userid + '&state=1&entryid=' + i;
            var sUrl = M.cfg.wwwroot + '/mod/techproject/ajax/updatecollapse.php?' + params;
            $.get(sUrl, function(data, status) {
                // $('#'+n).html(data);
            });
